...
int ConditionFunctions::luaConditionSetFormula(lua_State* L)
{
	...
	auto condition = dynamic_cast<ConditionSpeed*>(getUserdata<Condition>(L, 1));
	if (condition)
	{
...
int ConditionFunctions::luaConditionSetOutfit(lua_State* L)
{
	...
	auto condition = dynamic_cast<ConditionOutfit*>(getUserdata<Condition>(L, 1));
	if (condition)
	{
		condition->setOutfit(outfit);
		pushBoolean(L, true);
...
int ConditionFunctions::luaConditionAddDamage(lua_State* L)
{
	...
	auto condition = dynamic_cast<ConditionDamage*>(getUserdata<Condition>(L, 1));
	if (condition)
	{
