...
void Modules::executeOnRecvbyte(Player* player, NetworkMessage msg, uint8_t byte) const
{
	for (auto& it : recvbyteList)
	{
		...
		if (module.getEventType() == MODULE_TYPE_RECVBYTE && module.getRecvbyte() == byte && player->canRunModule(
			module.getRecvbyte()))
		{
...
bool Module::configureEvent(const pugi::xml_node& node)
{
	...
	if (tmpStr == "recvbyte")
	{
		const pugi::xml_attribute byteAttribute = node.attribute("byte");
		if (!byteAttribute)
		{
	...
	const pugi::xml_attribute delayAttribute = node.attribute("delay");
	if (delayAttribute)
	{
