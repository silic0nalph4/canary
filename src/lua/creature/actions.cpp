...
bool Actions::registerEvent(Event_ptr event, const pugi::xml_node& node) {
	...
	const Action_ptr action{static_cast<Action*>(event.release())};

	pugi::xml_attribute attr;
	if ((attr = node.attribute("itemid"))) {
		uint16_t id = pugi::cast<uint16_t>(attr.value());

		const auto result = useItemMap.emplace(id, std::move(*action));
		if (!result.second) {
		...
		const pugi::xml_attribute toIdAttribute = node.attribute("toid");
		if (!toIdAttribute) {
		...
		const auto result = uniqueItemMap.emplace(uid, std::move(*action));
		if (!result.second) {
			SPDLOG_WARN("[Actions::registerEvent] - Duplicate "
		...
		const pugi::xml_attribute toUidAttribute = node.attribute("touid");
		if (!toUidAttribute) {
		...
		const auto result = actionItemMap.emplace(aid, std::move(*action));
		if (!result.second) {
			SPDLOG_WARN("[Actions::registerEvent] - Duplicate "
		...
		const pugi::xml_attribute toAidAttribute = node.attribute("toaid");
		if (!toAidAttribute) {
...
bool Actions::registerLuaEvent(Action* event) {
	...
	if (action->getItemIdRange().size() > 0) {
		if (action->getItemIdRange().size() == 1) {
			const auto result = useItemMap.emplace(action->getItemIdRange().at(0), std::move(*action));
			if (!result.second) {
			...
			for (auto i = v.begin(); i != v.end(); i++) {
				const auto result = useItemMap.emplace(*i, std::move(*action));
				if (!result.second) {
					SPDLOG_WARN("[Actions::registerLuaEvent] - Duplicate "
		...
		if (action->getUniqueIdRange().size() == 1) {
			const auto result = uniqueItemMap.emplace(action->getUniqueIdRange().at(0), std::move(*action));
			if (!result.second) {
				SPDLOG_WARN("[Actions::registerLuaEvent] - Duplicate "
			...
			for (auto i = v.begin(); i != v.end(); i++) {
				const auto result = uniqueItemMap.emplace(*i, std::move(*action));
				if (!result.second) {
					SPDLOG_WARN("[Actions::registerLuaEvent] - Duplicate "
		...
		if (action->getActionIdRange().size() == 1) {
			const auto result = actionItemMap.emplace(action->getActionIdRange().at(0), std::move(*action));
			if (!result.second) {
				SPDLOG_WARN("[Actions::registerLuaEvent] - Duplicate "
			...
			for (auto i = v.begin(); i != v.end(); i++) {
				const auto result = actionItemMap.emplace(*i, std::move(*action));
				if (!result.second) {
					SPDLOG_WARN("[Actions::registerLuaEvent] Duplicate "
...
ReturnValue Actions::internalUseItem(Player* player, const Position& pos, uint8_t index, Item* item, bool isHotkey) {
	...
	if (Container* container = item->getContainer()) {
		...
		if (container->isRewardCorpse()) {
			...
			if (player->getGroup()->id >= account::GROUP_TYPE_GAMEMASTER || player->getAccountType() >=
...
bool Actions::useItemEx(Player* player, const Position& fromPos, const Position& toPos,
	...
	if (action->function) {
		if (action->function(player, item, fromPos, action->getTarget(player, creature, toPos, toStackPos), toPos,
		                     isHotkey)) {
			return true;
		}
		return false;
