	...
	if (!getRunning()) {
		...
		for (auto it = raidList.begin(), end = raidList.end(); it != end; ++it) {
			...
			if (now >= (getLastRaidEnd() + raid->getMargin())) {
				if (((MAX_RAND_RANGE * CHECK_RAIDS_INTERVAL) / raid->getInterval()) >= static_cast<uint32_t>(uniform_random(
					0, MAX_RAND_RANGE))) {
					setRunning(raid);
					raid->startRaid();
...
void Raid::startRaid() {
	...
	if (raidEvent) {
		...
		nextEventEvent = g_scheduler.addEvent(createSchedulerTask(raidEvent->getDelay(), [this, raidEvent] {
...
void Raid::executeRaidEvent(RaidEvent* raidEvent) {
	if (raidEvent->executeEvent()) {
		...
		if (newRaidEvent) {
			const uint32_t ticks = static_cast<uint32_t>(std::max<int32_t>(RAID_MINTICKS,
			                                                               newRaidEvent->getDelay() - raidEvent->getDelay()));
			nextEventEvent = g_scheduler.addEvent(createSchedulerTask(ticks, [this, newRaidEvent] {
				executeRaidEvent(newRaidEvent);
			}));
		}
		else {
			resetRaid();
...
bool AreaSpawnEvent::configureRaidEvent(const pugi::xml_node& eventNode) {
	...
	if ((attr = eventNode.attribute("radius"))) {
		const int32_t radius = pugi::cast<int32_t>(attr.value());
		Position centerPos;
	...
	for (const MonsterSpawn& spawn : spawnMonsterList) {
		...
		for (uint32_t i = 0; i < amount; ++i) {
			...
			for (int32_t tries = 0; tries < MAXIMUM_TRIES_PER_MONSTER; tries++) {
				...
				if (tile && !tile->isMoveableBlocking() && !tile->hasFlag(TILESTATE_PROTECTIONZONE) && tile->getTopCreature() ==
					nullptr && g_game.placeCreature(monster, tile->getPosition(), false, true)) {
					success = true;
					break;
				}
			}
...
bool ScriptEvent::configureRaidEvent(const pugi::xml_node& eventNode) {
	...
	auto scriptName = std::string(scriptAttribute.as_string());
