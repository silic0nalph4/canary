...
void Game::loadBoostedCreature()
{
	...
	const uint8_t today = ltm->tm_mday;
	if (date == today)
	{
	...
	else
	{
		...
		{
			const uint16_t random = normal_random(0, monsterlist.size());
			for (const auto& it : monsterlist)
			{
				if (k == random)
				{
					newrace = it.first;
...
void Game::start(ServiceManager* manager)
{
	...
	const int minutes = tms->tm_min;
	lightHour = (minutes * LIGHT_DAY_LENGTH) / 60;

	g_scheduler.addEvent(createSchedulerTask(EVENT_LIGHTINTERVAL_MS, [this] { checkLight(); }));
	...
	g_scheduler.addEvent(createSchedulerTask(EVENT_IMBUEMENTINTERVAL, [this] { checkImbuements(); }));
}
...
bool Game::loadScheduleEventFromXml()
{
	...
	const pugi::xml_parse_result result = doc.load_file("data/XML/events.xml");
	if (!result)
	{
	...
	const int16_t daysMath = ((timePtr->tm_year + 1900) * 365) + ((timePtr->tm_mon + 1) * 30) + (timePtr->tm_mday);
	...
	for (auto schedNode : doc.child("events").children())
	{
		...
		for (auto schedENode : schedNode.children())
		{
			...
			{
				const uint16_t exprate = pugi::cast<uint16_t>(schedENode.attribute("exprate").value());
			...
			{
				const uint16_t lootrate = pugi::cast<uint16_t>(schedENode.attribute("lootrate").value());
			...
			{
				const uint32_t spawnrate = pugi::cast<uint32_t>(schedENode.attribute("spawnrate").value());
			...
			{
				const uint16_t skillrate = pugi::cast<uint16_t>(schedENode.attribute("skillrate").value());
	...
	else
	{
		...
		if (hasBitSet(SLOTP_TWO_HAND, slotP))
		{
			...
			if (leftthing)
			{
				...
				if (slotLeft_item)
				{
					...
					ret = internalMoveItem(slotLeft_item->getParent(), player, 0, slotLeft_item, slotLeft_item->getItemCount(),
					                       nullptr);
				}
			}
			...
			if (rightthing)
			{
				...
				if (slotRight_Item)
				{
					if (newitemType.weaponType != WEAPON_DISTANCE || slotRight_Item->getWeaponType() != WEAPON_QUIVER)
						ret = internalMoveItem(slotRight_Item->getParent(), player, 0, slotRight_Item,
		...
		else if (hasBitSet(SLOTP_RING, slotP))
		{
			...
			if (ringthing)
			{
				...
				if (slotRing_Item)
				{
					ret = internalMoveItem(slotRing_Item->getParent(), player, 0, slotRing_Item, slotRing_Item->getItemCount(),
					                       nullptr);
		...
		else if (slotP == CONST_SLOT_RIGHT)
		{
			...
			if (rightthing)
			{
				...
				if (slotRight_Item)
				{
					...
					ret = internalMoveItem(slotRight_Item->getParent(), player, 0, slotRight_Item, slotRight_Item->getItemCount(),
					                       nullptr);
				}
				else
				{
				...
				if (slotLeft_item)
				{
					...
					if (hasBitSet(SLOTP_TWO_HAND, it.slotPosition))
					{
						if (newitemType.weaponType != WEAPON_QUIVER || slotLeft_item->getWeaponType() != WEAPON_DISTANCE)
							ret = internalMoveItem(slotLeft_item->getParent(), player, 0, slotLeft_item,
		...
		else if (slotP)
		{
			if (newitemType.stackable)
			{
				...
				if (ammothing)
				{
					...
					if (ammoItem)
					{
						const ObjectCategory_t category = getObjectCategory(ammoItem);
				...
				const ReturnValue ret2 = player->queryAdd(slotP, *item, item->getItemCount(), 0);
                              ...
                              StackPosType_t type) const
{
	...
	//inventory
	const auto slot = static_cast<Slots_t>(pos.y);
...
bool Game::internalPlaceCreature(Creature* creature, const Position& pos, bool extendedPos /*=false*/,
                                 bool forced /*= false*/, bool creatureCheck /*= false*/)
{
...
bool Game::removeCreature(Creature* creature, bool isLogout/* = true*/)
{
	...
	for (Creature* spectator : spectators)
	{
		...
		{
			oldStackPosVector.push_back(player->canSeeCreature(creature)
...
void Game::playerTeleport(uint32_t playerId, const Position& newPosition)
{
	...
	const ReturnValue returnValue = g_game.internalTeleport(player, newPosition, false);
	...
	if (Creature* movingCreature = thing->getCreature())
	{
		                                  ...
		                                  player->getPosition()))
		{
			SchedulerTask* task = createSchedulerTask(
				...
				[this, capture0 = player->getID(), capture1 = movingCreature->getID(), capture2 =
					movingCreature->getPosition(), capture3 = tile->getPosition()]
				{
	...
	if (!Position::areInRange<1, 1, 0>(movingCreatureOrigPos, player->getPosition()))
	{
		...
		if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true))
		{
			g_dispatcher.addTask(createTask(
				[this, capture0 = player->getID(), listDir] { playerAutoWalk(capture0, listDir); }));

			SchedulerTask* task = createSchedulerTask(600, [this, capture0 = player->getID(), capture1 = movingCreature->
				                                          getID(), movingCreatureOrigPos, capture2 = toTile->getPosition()]
			                                          {
				                                          playerMoveCreatureByID(
					                                          capture0, capture1, movingCreatureOrigPos, capture2);
			                                          });
...
void Game::playerMoveItem(Player* player, const Position& fromPos,
                          uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count, Item* item,
                          Cylinder* toCylinder)
{
	if (!player->canDoAction())
	{
		...
		SchedulerTask* task = createSchedulerTask(delay,
		                                          [this, capture0 = player->getID(), fromPos, spriteId, fromStackPos, toPos,
			                                          count]
		                                          {
			                                          playerMoveItemByPlayerID(
				                                          capture0, fromPos, spriteId, fromStackPos, toPos, count);
		                                          });
	...
	if (!Position::areInRange<1, 1>(playerPos, mapFromPos))
	{
		...
		if (player->getPathTo(item->getPosition(), listDir, 0, 1, true, true))
		{
			...
			SchedulerTask* task = createSchedulerTask(400,
			                                          [this, capture0 = player->getID(), fromPos, spriteId, fromStackPos,
				                                          toPos, count]
			                                          {
				                                          playerMoveItemByPlayerID(
					                                          capture0, fromPos, spriteId, fromStackPos, toPos, count);
	...
	if (item->isHangable() && toCylinderTile->hasFlag(TILESTATE_SUPPORTS_HANGABLE))
	{
		...
		if (!Position::areInRange<1, 1, 0>(playerPos, mapToPos))
		{
			...
			{
				g_dispatcher.addTask(createTask([this, capture0 = player->getID(), listDir]
				{
					playerAutoWalk(capture0, listDir);
				}));

				SchedulerTask* task = createSchedulerTask(400,
				                                          [this, capture0 = player->getID(), itemPos, spriteId, itemStackPos,
					                                          toPos, count]
				                                          {
					                                          playerMoveItemByPlayerID(
	...
	if (toCylinder->getContainer() != nullptr &&
	...
	if (item->isWrapable())
	{
		auto toHouseTile = dynamic_cast<HouseTile*>(map.getTile(mapToPos));
		auto fromHouseTile = dynamic_cast<HouseTile*>(map.getTile(mapFromPos));
                                   ...
                                   Item* tradeItem/* = nullptr*/)
{
	...
	if (ret == RETURNVALUE_NEEDEXCHANGE)
	{
		...
		if (ret == RETURNVALUE_NOERROR)
		{
			...
			const ReturnValue retExchangeMaxCount = fromCylinder->queryMaxCount(
	...
	if (quiver && quiver->getWeaponType() == WEAPON_QUIVER && quiver->getHoldingPlayer() && quiver->getHoldingPlayer()->
		getThing(CONST_SLOT_RIGHT) == quiver)
	{
	...
	if (fromCylinder && actor && toCylinder)
	{
		...
		if (Player* player = actor->getPlayer())
		{
			...
			if (it.corpseType != RACE_NONE && toCylinder->getContainer()->getTopParent() == player && item->
				getIsLootTrackeable())
			{
...
{
	uint32_t remainderCount = 0;
	ReturnValue ret = internalAddItem(player, item, slot, 0, false, remainderCount);
	if (remainderCount != 0)
	{
...
Item* Game::transformItem(Item* item, uint16_t newId, int32_t newCount /*= -1*/)
{
	...
	if (curType.type == newType.type)
	{
		...
		{
			const uint32_t currentDuration = item->getDuration();
...
void Game::internalQuickLootCorpse(Player* player, Container* corpse)
{
	...
	{
		const uint32_t worth = item->getWorth();
		const uint16_t baseCount = item->getItemCount();
		const ObjectCategory_t category = getObjectCategory(item);

		const ReturnValue ret = internalQuickLootItem(player, item, category);
		...
		const bool success = ret == RETURNVALUE_NOERROR;
		if (worth != 0)
		{
	...
	{
		ss.str(std::string());
		ss << "Attention! The container assigned to category " << getObjectCategoryName(shouldNotifyNotEnoughRoom) <<
			" is full.";
	}
	else
	{
...
ReturnValue Game::internalQuickLootItem(Player* player, Item* item,
                                        ObjectCategory_t category /* = OBJECTCATEGORY_DEFAULT*/)
{
	...
	{
		uint16_t baseId = lootContainer->getID();
	}
	...
	do
	{
		...
		if (item->getParent())
		{
			// Stash retrive dont have parent cylinder.
			ret = internalMoveItem(item->getParent(), lootContainer, INDEX_WHEREEVER, item, item->getItemCount(), &moveItem,
			                       0, player);
		}
		else
		{
		...
		const bool quickFallback = (player->quickLootFallbackToMainContainer);
		const bool noFallback = fallbackConsumed || !quickFallback;
...
ObjectCategory_t Game::getObjectCategory(const Item* item)
{
	...
	{
		// if it's a weapon/shield should have been parsed earlier
                           ...
                           const Position& toPos, uint8_t toStackPos, uint16_t toSpriteId)
{
	...
	if (ret != RETURNVALUE_NOERROR)
	{
		if (ret == RETURNVALUE_TOOFARAWAY)
		{
			...
			if (player->getPathTo(walkToPos, listDir, 0, 1, true, true))
			{
				...
				SchedulerTask* task = createSchedulerTask(400,
				                                          [this, playerId, itemPos, itemStackPos, fromSpriteId, toPos,
					                                          toStackPos, toSpriteId]
				                                          {
					                                          playerUseItemEx(playerId, itemPos, itemStackPos, fromSpriteId,
	...
	if (!canDoAction)
	{
		...
		SchedulerTask* task = createSchedulerTask(delay,
		                                          [this, playerId, fromPos, fromStackPos, fromSpriteId, toPos, toStackPos,
			                                          toSpriteId]
		                                          {
			                                          playerUseItemEx(playerId, fromPos, fromStackPos, fromSpriteId, toPos,
			                                                          toStackPos, toSpriteId);
		                                          });
		if (it.isRune() || it.type == ITEM_TYPE_POTION)
		{
                         ...
                         uint8_t index, uint16_t spriteId)
{
	...
	if (!canDoAction)
	{
		...
		SchedulerTask* task = createSchedulerTask(delay, [this, playerId, pos, stackPos, index, spriteId]
		{
			playerUseItem(playerId, pos, stackPos, index, spriteId);
		});
		if (it.isRune() || it.type == ITEM_TYPE_POTION)
                                 ...
                                 uint16_t spriteId)
{
	...
	if (ret != RETURNVALUE_NOERROR)
	{
		if (ret == RETURNVALUE_TOOFARAWAY)
		{
			...
			if (fromPos.x != 0xFFFF && Position::areInRange<1, 1, 0>(fromPos, player->getPosition()) && !Position::areInRange<
				1, 1, 0>(fromPos, toPos))
			{
				Item* moveItem = nullptr;
				ret = internalMoveItem(item->getParent(), player, INDEX_WHEREEVER, item, item->getItemCount(), &moveItem);
	...
	if (!canDoAction)
	{
		...
		SchedulerTask* task = createSchedulerTask(delay, [this, playerId, fromPos, fromStackPos, creatureId, spriteId]
		{
			playerUseWithCreature(playerId, fromPos, fromStackPos, creatureId,
			                      spriteId);
		});
...
void Game::playerMoveUpContainer(uint32_t playerId, uint8_t cid)
{
	...
	if (!parentContainer)
	{
		...
		if (it == browseFields.end())
		{
			...
			g_scheduler.addEvent(createSchedulerTask(30000, [this, capture0 = tile->getPosition()]
			{
				decreaseBrowseFieldRef(capture0);
			}));
		}
		else
		{
	...
	{
		const uint16_t indexContainer = std::floor(parentContainer->getThingIndex(container) / parentContainer->capacity())
...
void Game::playerRotateItem(uint32_t playerId, const Position& pos, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if (pos.x != 0xFFFF && !Position::areInRange<1, 1, 0>(pos, player->getPosition()))
	{
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true))
		{
			...
			SchedulerTask* task = createSchedulerTask(400, [this, playerId, pos, stackPos, spriteId]
			{
...
void Game::playerConfigureShowOffSocket(uint32_t playerId, const Position& pos, uint8_t stackPos,
...
void Game::playerSetShowOffSocket(uint32_t playerId, Outfit_t& outfit, const Position& pos, uint8_t stackPos,
                                  const uint16_t spriteId, uint8_t podiumVisible, uint8_t direction)
{
	Player* player = getPlayerByID(playerId);
	if (!player || pos.x == 0xFFFF)
	{
	...
	auto tile = dynamic_cast<Tile*>(item->getParent());
	if (!tile)
	{
		player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
		return;
...
void Game::playerWrapableItem(uint32_t playerId, const Position& pos, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if ((item->getHoldingPlayer() && item->getID() == 26054) || (tile->hasFlag(TILESTATE_IMMOVABLEBLOCKSOLID) && !item->
	...
	if (attr != nullptr)
	{
		const uint32_t tmp = static_cast<uint32_t>(boost::get<int64_t>(attr->value));
	...
	if (item->isWrapable() && item->getID() != TRANSFORM_BOX_ID)
	{
		...
		const uint16_t oldItemID = item->getID();
	...
	{
		const uint16_t hiddenCharges = item->getDate();
...
void Game::playerStowItem(Player* player, const Position& pos, uint16_t spriteId, uint8_t stackpos, uint8_t count,
                          bool allItems)
{
	if (!player->isPremium())
	{
...
void Game::playerStashWithdraw(Player* player, uint16_t spriteId, uint32_t count, uint8_t)
{
	...
	const int32_t NDSlots = ((freeSlots) - (count < 100 ? 1 : (count / 100)));
	...
	std::stringstream ss;
	const uint32_t WithdrawCount = (SlotsWith > capWith ? capWith : SlotsWith);
	const uint32_t NoWithdrawCount = (noSlotsWith < noCapWith ? noCapWith : noSlotsWith);
                              ...
                              uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	if (tradeItem->getClientID() != spriteId || !tradeItem->isPickupable() || tradeItem->hasAttribute(
	...
	{
		if (auto houseTile = dynamic_cast<HouseTile*>(tradeItem->getTile()))
		{
	...
	if (!Position::areInRange<1, 1>(tradeItemPosition, playerPosition))
	{
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true))
		{
			...
			SchedulerTask* task = createSchedulerTask(400, [this, playerId, pos, stackPos, tradePlayerId, spriteId]
			{
	...
	if (tradePartner->getTradeState() == TRADE_ACCEPT)
	{
		...
		if (ret1 == RETURNVALUE_NOERROR && ret2 == RETURNVALUE_NOERROR)
		{
			...
			if (ret1 == RETURNVALUE_NOERROR && ret2 == RETURNVALUE_NOERROR)
			{
				...
				const uint32_t count1 = tradeItem1->getItemCount();
				const uint32_t count2 = tradeItem2->getItemCount();

				ret1 = internalMoveItem(cylinder1, tradePartner, INDEX_WHEREEVER, tradeItem1, count1, nullptr,
...
void Game::playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer, uint8_t index)
{
	...
	const int32_t lookDistance = std::max<int32_t>(
...
void Game::playerQuickLoot(uint32_t playerId, const Position& pos, uint16_t spriteId, uint8_t stackPos,
                           Item* defaultItem)
{
	Player* player = getPlayerByID(playerId);
	if (!player)
	...
	if (!player->canDoAction())
	{
		const uint32_t delay = player->getNextActionTime();
		SchedulerTask* task = createSchedulerTask(delay,
		                                          [this, capture0 = player->getID(), pos, spriteId, stackPos, defaultItem]
		                                          {
			                                          playerQuickLoot(capture0, pos, spriteId, stackPos, defaultItem);
		                                          });
		player->setNextActionTask(task);
		return;
	}
	...
	if (pos.x != 0xffff)
	{
		if (!Position::areInRange<1, 1, 0>(pos, player->getPosition()))
		{
			...
			if (player->getPathTo(pos, listDir, 0, 1, true, true))
			{
				...
				SchedulerTask* task = createSchedulerTask(0, [this, capture0 = player->getID(), pos, spriteId, stackPos,
					                                          defaultItem]
				                                          {
					                                          playerQuickLoot(capture0, pos, spriteId, stackPos, defaultItem);
				                                          });
...
void Game::playerSetLootContainer(uint32_t playerId, ObjectCategory_t category, const Position& pos, uint16_t spriteId,
                                  uint8_t stackPos)
{
	Player* player = getPlayerByID(playerId);
	if (!player || pos.x != 0xffff)
...
void Game::playerQuickLootBlackWhitelist(uint32_t playerId, QuickLootFilter_t filter, std::vector<uint16_t> clientIds)
{
	...
	player->quickLootListClientIds = std::move(clientIds);
}
...
void Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	g_dispatcher.addTask(createTask([this, capture0 = player->getID()] { updateCreatureWalk(capture0); }));
}
                         ...
                         const std::string& text)
{
	...
	if (type == TALKTYPE_PRIVATE_RED_TO && (player->hasFlag(PlayerFlag_CanTalkRedPrivate) || player->getAccountType() >=
...
bool Game::internalCreatureSay(Creature* creature, SpeakClasses type, const std::string& text,
                               bool ghostMode, SpectatorHashSet* spectatorsPtr/* = nullptr*/,
...
void Game::checkCreatures(size_t index)
{
	g_scheduler.addEvent(createSchedulerTask(EVENT_CHECK_CREATURE_INTERVAL,
	                                         [this, capture0 = (index + 1) % EVENT_CREATURECOUNT]
	                                         {
                          ...
                          bool checkArmor, bool field)
{
	...
	if (damage.primary.type != COMBAT_NONE)
	{
		...
		{
			const uint32_t primaryReflect = target->getMonster()->getReflectValue(damage.primary.type);
			if (primaryReflect > 0)
			{
		...
		if (attacker && target->getMonster())
		{
			const uint32_t primaryHealing = target->getMonster()->getHealingCombatValue(damage.primary.type);
			if (primaryHealing > 0)
			{
		...
		primaryBlockType = target->blockHit(attacker, damage.primary.type, damage.primary.value, checkDefense, checkArmor,
		                                    field);
	...
	if (damage.secondary.type != COMBAT_NONE)
	{
		...
		if (attacker && target->getMonster())
		{
			const uint32_t secondaryReflect = target->getMonster()->getReflectValue(damage.secondary.type);
		...
		if (attacker && target->getMonster())
		{
			const uint32_t secondaryHealing = target->getMonster()->getHealingCombatValue(damage.secondary.type);
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage, bool isEvent /*= false*/)
{
	...
	if (damage.primary.value > 0)
	{
		...
		if (attackerPlayer && targetPlayer && attackerPlayer->getSkull() == SKULL_BLACK && attackerPlayer->
			getSkullClient(targetPlayer) == SKULL_NONE)
		{
			return false;
		}
	...
	else
	{
		...
		if (!isEvent)
		{
			g_events->eventCreatureOnDrainHealth(target, attacker, damage.primary.type, damage.primary.value,
		...
		if (!damage.extension && attacker && attacker->getMonster() && targetPlayer)
		{
			...
			if (mType)
			{
				...
				if (activeCharm != CHARM_NONE && activeCharm != CHARM_CLEANSE)
				{
					...
					{
						if (g_bestiary.parseCharmCombat(charm, targetPlayer, attacker,
					...
					else
					{
						if (spectatorMessage.empty())
						{
							...
							if (attacker)
							{
								...
								{
									ss << (targetPlayer
										       ? (targetPlayer->getSex() == PLAYERSEX_FEMALE ? "her own attack" : "his own attack")
										       : "its own attack");
								}
								else
								{
...
{
	const uint8_t healthPercent = std::ceil(
		(static_cast<double>(target->getHealth()) / std::max<int32_t>(target->getMaxHealth(), 1)) * 100);
...
void Game::addPlayerMana(const Player* target)
{
	...
	{
		const uint8_t manaPercent = std::ceil(
			(static_cast<double>(target->getMana()) / std::max<int32_t>(target->getMaxMana(), 1)) * 100);
...
void Game::addDistanceEffect(const SpectatorHashSet& spectators, const Position& fromPos, const Position& toPos,
                             uint8_t effect)
{
	for (Creature* spectator : spectators)
	{
			...
			for (int32_t s = STAT_FIRST; s <= STAT_LAST; ++s)
			{
				...
				if (it.abilities && it.abilities->statsPercent[s] != 0)
				{
					...
					player->setVarStats(static_cast<stats_t>(s),
					                    -static_cast<int32_t>(player->getDefaultStats(static_cast<stats_t>(s)) * ((it.abilities->
...
void Game::checkDecay()
{
	...
	while (it != end)
	{
		...
		{
			it = decayItems[bucket].erase(it);
			const size_t newBucket = (bucket + ((duration + EVENT_DECAYINTERVAL / 2) / 1000)) % EVENT_DECAY_BUCKETS;
			if (newBucket == bucket)
			{
				internalDecayItem(item);
				ReleaseItem(item);
...
void Game::checkImbuements()
{
	...
	const size_t bucket = (lastImbuedBucket + 1) % EVENT_IMBUEMENT_BUCKETS;
	...
	while (it != end)
	{
		...
		const uint8_t slots = Item::items[item->getID()].imbuingSlots;
		for (uint8_t slot = 0; slot < slots; slot++)
		{
			const uint32_t info = item->getImbuement(slot);
			const int32_t duration = info >> 8;
			const int32_t newDuration = std::max(0, (duration - (EVENT_IMBUEMENTINTERVAL * EVENT_IMBUEMENT_BUCKETS) / 690));
		...
		const int32_t index = player ? player->getThingIndex(item) : -1;
		...
		for (uint8_t slot = 0; slot < slots; slot++)
		{
			...
			const int32_t decreaseTime = std::min<int32_t>((EVENT_IMBUEMENTINTERVAL * EVENT_IMBUEMENT_BUCKETS) / 690,
			                                               duration);
			duration -= decreaseTime;

			const int32_t id = info & 0xFF;
			const int64_t newinfo = (duration << 8) | id;
...
{
	const auto it = BestiaryList.find(raceid);
...
void Game::updatePremium(account::Account& account)
{
	...
	if (rem_days != 0)
	{
		...
		{
			const uint32_t days = (timeNow - last_day) / 86400;
			if (days > 0)
			{
				if (days >= rem_days)
				{
				...
				else
				{
					...
					const time_t remainder = (timeNow - last_day) % 86400;
...
void Game::saveMotdNum() const
{
	...
	query.str(std::string());
	query << "UPDATE `server_config` SET `value` = '" << transformToSHA1(g_config.getString(MOTD)) <<
...
void Game::loadPlayersRecord()
{
	...
	const DBResult_ptr result = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'players_record'");
	if (result)
	{
...
void Game::playerEnableSharedPartyExperience(uint32_t playerId, bool sharedExpActive)
{
	...
	if (!party || (player->hasCondition(CONDITION_INFIGHT) && playerTile && !playerTile->
                                         ...
                                         uint16_t page)
{
	...
	{
	case CYCLOPEDIA_CHARACTERINFO_BASEINFORMATION: player->sendCyclopediaCharacterBaseInformation();
		break;
	case CYCLOPEDIA_CHARACTERINFO_GENERALSTATS: player->sendCyclopediaCharacterGeneralStats();
		break;
	case CYCLOPEDIA_CHARACTERINFO_COMBATSTATS: player->sendCyclopediaCharacterCombatStats();
		break;
	case CYCLOPEDIA_CHARACTERINFO_RECENTDEATHS:
		{
			std::ostringstream query;
			const uint32_t offset = static_cast<uint32_t>(page - 1) * entriesPerPage;
			query <<
				"SELECT `time`, `level`, `killed_by`, `mostdamage_by`, (select count(*) FROM `player_deaths` WHERE `player_id` = "
				<< playerGUID << ") as `entries` FROM `player_deaths` WHERE `player_id` = " << playerGUID <<
				" ORDER BY `time` DESC LIMIT " << offset << ", " << entriesPerPage;

			uint32_t playerID = player->getID();
			const std::function<void(DBResult_ptr, bool)> callback = [playerID, page, entriesPerPage
				](const DBResult_ptr& result, bool)
			{
	...
	case CYCLOPEDIA_CHARACTERINFO_RECENTPVPKILLS:
		{
			...
			const uint32_t offset = static_cast<uint32_t>(page - 1) * entriesPerPage;
			query <<
				"SELECT `d`.`time`, `d`.`killed_by`, `d`.`mostdamage_by`, `d`.`unjustified`, `d`.`mostdamage_unjustified`, `p`.`name`, (select count(*) FROM `player_deaths` WHERE ((`killed_by` = "
				<< escapedName << " AND `is_player` = 1) OR (`mostdamage_by` = " << escapedName <<
				" AND `mostdamage_is_player` = 1))) as `entries` FROM `player_deaths` AS `d` INNER JOIN `players` AS `p` ON `d`.`player_id` = `p`.`id` WHERE ((`d`.`killed_by` = "
				<< escapedName << " AND `d`.`is_player` = 1) OR (`d`.`mostdamage_by` = " << escapedName <<
				" AND `d`.`mostdamage_is_player` = 1)) ORDER BY `time` DESC LIMIT " << offset << ", " << entriesPerPage;

			uint32_t playerID = player->getID();
			const std::function<void(DBResult_ptr, bool)> callback = [playerID, page, entriesPerPage](const DBResult_ptr&
				result, bool)
			{
				Player* player = g_game.getPlayerByID(playerID);
				if (!player)
	...
	case CYCLOPEDIA_CHARACTERINFO_ACHIEVEMENTS: player->sendCyclopediaCharacterAchievements();
		break;
	case CYCLOPEDIA_CHARACTERINFO_ITEMSUMMARY: player->sendCyclopediaCharacterItemSummary();
		break;
	case CYCLOPEDIA_CHARACTERINFO_OUTFITSMOUNTS: player->sendCyclopediaCharacterOutfitsMounts();
		break;
	case CYCLOPEDIA_CHARACTERINFO_STORESUMMARY: player->sendCyclopediaCharacterStoreSummary();
		break;
	case CYCLOPEDIA_CHARACTERINFO_INSPECTION: player->sendCyclopediaCharacterInspection();
		break;
	case CYCLOPEDIA_CHARACTERINFO_BADGES: player->sendCyclopediaCharacterBadges();
		break;
	case CYCLOPEDIA_CHARACTERINFO_TITLES: player->sendCyclopediaCharacterTitles();
		break;
	default: player->sendCyclopediaCharacterNoData(characterInfoType, 1);
		break;
	}
}

void Game::playerHighscores(Player* player, HighscoreType_t type, uint8_t category, uint32_t vocation,
                            const std::string&, uint16_t page, uint8_t entriesPerPage)
{
	...
	switch (category)
	{
	case HIGHSCORE_CATEGORY_FIST_FIGHTING: categoryName = "skill_fist";
		break;
	case HIGHSCORE_CATEGORY_CLUB_FIGHTING: categoryName = "skill_club";
		break;
	case HIGHSCORE_CATEGORY_SWORD_FIGHTING: categoryName = "skill_sword";
		break;
	case HIGHSCORE_CATEGORY_AXE_FIGHTING: categoryName = "skill_axe";
		break;
	case HIGHSCORE_CATEGORY_DISTANCE_FIGHTING: categoryName = "skill_dist";
		break;
	case HIGHSCORE_CATEGORY_SHIELDING: categoryName = "skill_shielding";
		break;
	case HIGHSCORE_CATEGORY_FISHING: categoryName = "skill_fishing";
		break;
	case HIGHSCORE_CATEGORY_MAGIC_LEVEL: categoryName = "maglevel";
		break;
	default:
		{
	...
	if (type == HIGHSCORE_GETENTRIES)
	{
		...
		query << "SELECT *, @row AS `entries`, " << page <<
			" AS `page` FROM (SELECT *, (@row := @row + 1) AS `rn` FROM (SELECT `id`, `name`, `level`, `vocation`, `" <<
			categoryName << "` AS `points`, @curRank := IF(@prevRank = `" << categoryName << "`, @curRank, IF(@prevRank := `"
			<< categoryName <<
	...
	else if (type == HIGHSCORE_OURRANK)
	{
		...
		query << "SELECT *, @row AS `entries`, (@ourRow DIV " << entriesStr <<
			") + 1 AS `page` FROM (SELECT *, (@row := @row + 1) AS `rn`, @ourRow := IF(`id` = " << player->getGUID() <<
			", @row - 1, @ourRow) AS `rw` FROM (SELECT `id`, `name`, `level`, `vocation`, `" << categoryName <<
			"` AS `points`, @curRank := IF(@prevRank = `" << categoryName << "`, @curRank, IF(@prevRank := `" << categoryName
			<<
			"`, @curRank + 1, @curRank + 1)) AS `rank` FROM `players` `p`, (SELECT @curRank := 0, @prevRank := NULL, @row := 0, @ourRow := 0) `r` WHERE `group_id` < "
			<< static_cast<int>(account::GROUP_TYPE_GAMEMASTER) << " ORDER BY `" << categoryName << "` DESC) `t`";
		if (vocation != 0xFFFFFFFF)
		{
			bool firstVocation = true;
		...
		query << ") `T` WHERE `rn` > ((@ourRow DIV " << entriesStr << ") * " << entriesStr <<
			") AND `rn` <= (((@ourRow DIV " << entriesStr << ") * " << entriesStr << ") + " << entriesStr << ")";
	}

	uint32_t playerID = player->getID();
	std::function<void(DBResult_ptr, bool)> callback = [playerID, category, vocation, entriesPerPage](const DBResult_ptr&
		result, bool)
	{
		Player* player = g_game.getPlayerByID(playerID);
		if (!player)
		...
		const uint16_t page = result->getNumber<uint16_t>("page");
		uint32_t pages = result->getNumber<uint32_t>("entries");
		pages += entriesPerPage - 1;
		pages /= entriesPerPage;
		...
		do
		{
			...
			characters.emplace_back(std::move(result->getString("name")), result->getNumber<uint64_t>("points"),
			                        result->getNumber<uint32_t>("id"), result->getNumber<uint32_t>("rank"),
			                        result->getNumber<uint16_t>("level"), characterVocation);
		}
		while (result->next());
...
void Game::playerReportRuleViolationReport(uint32_t playerId, const std::string& targetName, uint8_t reportType,
                                   ...
                                   bool anonymous) // Limit of 64k of items to create offer
{
	...
	const uint64_t calcFee = (price / 100.) * amount;
	const uint32_t minFee = std::min<uint32_t>(100000, calcFee);
	const uint32_t fee = std::max<uint32_t>(20, minFee);
	...
	const auto ColorItem = itemsPriceMap.find(it.id);
                               ...
                               const std::string& additionalInfo /* ="" */)
{
	...
	if (player)
	{
		...
		if (offer->type == ITEM || offer->type == STACKABLE_ITEM || offer->type == WRAP_ITEM)
		{
			...
			if (thing == nullptr)
			{
				player->sendStoreError(STORE_ERROR_NETWORK,
				                       "We cannot locate your store inbox, try again after relog and if this error persists, contact the system administrator.");
				return;
			}
			...
			uint32_t requiredSlots = (tmp->type == ITEM || tmp->type == WRAP_ITEM)
				                         ? tmp->count
				                         : (tmp->count % 100)
				                         ? static_cast<uint32_t>(tmp->count / 100) + 1
				                         : static_cast<uint32_t>(tmp->count) / 100;
		...
		else if (offer->type == NAMECHANGE)
		{
			...
			{
				// client didn't sent the new name yet, request additionalInfo
		...
		else if (offer->type == SEXCHANGE)
		{
			...
			for (auto outfit : player->outfits)
			{
				// adding all outfits of the oposite sex.
				const Outfit* opositeSexOutfit = Outfits::getInstance().getOpositeSexOutfitByLookType(
					playerSex, outfit.lookType);

				if (opositeSexOutfit)
				{
					player->addOutfit(opositeSexOutfit->lookType, 0);
					// since addons could have different recipes, we can't add automatically
				}
			}
		...
		{
			auto premiumTimeOffer = (PremiumTimeOffer*)offer;
			account.LoadAccountDB(player->getAccount());
			account.RemoveCoins(offer->price);
			account.RegisterCoinsTransaction(account::COIN_REMOVE, offer->price,
			                                 offer->name);
		...
		{
			auto tpOffer = (TeleportOffer*)offer;
			if (player->canLogout())
			{
				Position toPosition;
				Position fromPosition = player->getPosition();
				if (tpOffer->position.x == 0 || tpOffer->position.y == 0 || tpOffer->position.z == 0)
				{
					//temple teleport
			...
			{
				player->sendStoreError(STORE_ERROR_PURCHASE,
				                       "Your character has some teleportation block at the moment and cannot be teleported.");
			}
		}
		else if (offer->type == BLESSING)
		{
			auto blessingOffer = (BlessingOffer*)offer;

			uint8_t blessingsToAdd = 0;
			for (uint8_t bless : blessingOffer->blessings)
			{
				if (player->hasBlessing(bless))
				{
					// player already has this bless
                              ...
                              const std::string& receiverName, uint32_t amount)
{
	...
	else
	{
		...
		if (sender->getAccount() == receiver->getAccount())
		{
...
void Game::playerAnswerModalWindow(uint32_t playerId, uint32_t modalWindowId, uint8_t button, uint8_t choice)
{
	...
	if (modalWindowId == std::numeric_limits<uint32_t>::max())
	{
		if (button == 1)
		{
			if (choice == SKILL_SWORD || choice == SKILL_AXE || choice == SKILL_CLUB || choice == SKILL_DISTANCE || choice ==
				SKILL_MAGLEVEL)
			{
...
void Game::internalRemoveItems(const std::vector<Item*>& itemList, uint32_t amount, bool stackable)
{
	if (stackable)
	{
		for (Item* item : itemList)
...
bool Game::hasEffect(uint8_t effectId)
{
	...
	{
		const auto effect = static_cast<MagicEffectClasses>(i);
		if (effect == effectId)
		{
			return true;
		}
...
bool Game::hasDistanceEffect(uint8_t effectId)
{
	...
	{
		const auto effect = static_cast<ShootType_t>(i);
		if (effect == effectId)
		{
			return true;
		}
