...
public:
	...
	void playerCyclopediaCharacterInfo(Player* player, uint32_t characterID,
	                                   CyclopediaCharacterInfoType_t characterInfoType, uint16_t entriesPerPage,
	                                   uint16_t page);

	void playerHighscores(Player* player, HighscoreType_t type, uint8_t category, uint32_t vocation,
	                      const std::string& worldName, uint16_t page, uint8_t entriesPerPage);
	...
	void playerMoveCreature(Player* playerId, Creature* movingCreature, const Position& movingCreatureOrigPos,
	                        Tile* toTile);
	...
	void playerStowItem(Player* player, const Position& pos, uint16_t spriteId, uint8_t stackpos, uint8_t count,
	                    bool allItems);
	...
	void playerConfigureShowOffSocket(uint32_t playerId, const Position& pos, uint8_t stackPos, uint16_t spriteId);
	void playerSetShowOffSocket(uint32_t playerId, Outfit_t& outfit, const Position& pos, uint8_t stackPos,
	                            uint16_t spriteId, uint8_t podiumVisible, uint8_t direction);
	...
	void playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t spriteId, uint16_t amount, uint32_t price,
	                             bool anonymous);
