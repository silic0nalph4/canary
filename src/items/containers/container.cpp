...
Container::Container(uint16_t initType, uint16_t initSize, bool initUnlocked /*= true*/,
...
Container::Container(Tile* tile) : Container(ITEM_BROWSEFIELD, 30, false, true)
{
	...
	if (itemVector)
	{
		...
		{
			if (((item->getContainer() || item->hasProperty(CONST_PROP_MOVEABLE)) || (item->isWrapable() && !item->
				hasProperty(CONST_PROP_MOVEABLE) && !item->hasProperty(CONST_PROP_BLOCKPATH))) && !item->hasAttribute(
...
{
	auto clone = static_cast<Container*>(Item::clone());
	for (Item* item : itemlist)
	{
	...
	for (auto item : itemlist)
	{
		if (item->getContainer() != nullptr)
		{
			...
			for (const auto subContItem : subContainer)
			{
				...
				toReturnList.push_back(
					std::pair<Item*, uint32_t>(containerItem, static_cast<uint32_t>(containerItem->getItemCount())));
			}
		}
...
{
	const bool ret = Item::unserializeItemNode(loader, node, propStream);
	if (!ret)
	{
		return false;
	}
	...
	for (auto& itemNode : node.children)
	{
		...
		Item* item = CreateItem(itemPropStream);
		if (!item)
		{
			return false;
		}
...
{
	totalWeight += diff;
	auto parentContainer = this; // credits: SaiyansKing
                                ...
                                uint32_t flags, Creature* actor/* = nullptr*/) const
{
	...
	{
		const uint32_t maxItem = static_cast<uint32_t>(g_config.getNumber(MAX_ITEM));
		if (const Container* addContainer = item->getContainer())
		{
			...
			const uint32_t maxContainer = static_cast<uint32_t>(g_config.getNumber(MAX_CONTAINER));
                                   ...
                                   Creature* actor /*= nullptr */) const
{
	...
	auto houseTile = dynamic_cast<const HouseTile*>(getTopParent());
	if (houseTile)
	{
                                      ...
                                      uint32_t& flags)
{
	...
	if (index == 254 /*move up*/)
	{
		...
		*destItem = nullptr;
		...
		auto parentContainer = dynamic_cast<Container*>(getParent());
		if (parentContainer)
		{
	...
	if (index != INDEX_WHEREEVER)
	{
		...
		auto subCylinder = dynamic_cast<Cylinder*>(*destItem);
		if (subCylinder)
		{
			index = INDEX_WHEREEVER;
			*destItem = nullptr;
