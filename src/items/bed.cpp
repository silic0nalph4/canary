...
BedItem* BedItem::getNextBedItem() const {
	const Direction dir = items[id].bedPartnerDir;
...
bool BedItem::trySleep(Player* player) {
	...
	if (sleeperGUID != 0) {
		if (items[id].transformToFree != 0 && house->getOwner() == player->getGUID()) {
			wakeUp(nullptr);
		}
...
bool BedItem::sleep(Player* player) {
	...
	g_scheduler.addEvent(createSchedulerTask(SCHEDULER_MINTICKS, [ObjectPtr = &g_game, playerId] {
