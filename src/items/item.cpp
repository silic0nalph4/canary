...
{
	const int64_t slotid = IMBUEMENT_SLOT + slot;
	const ItemAttributes::CustomAttribute* attr = getCustomAttribute(slotid);
	if (attr)
	{
		const uint32_t info = static_cast<uint32_t>(boost::get<int64_t>(attr->value));
		if (info << 8)
			return info;
	}
...
{
	const int64_t slotid = IMBUEMENT_SLOT + slot;
	auto key = boost::lexical_cast<std::string>(slotid);
...
{
	const auto isContainerAndHasSomethingInside = (getContainer() != nullptr) && (getContainer()->getItemList().size() >
		0);
			...
			if (it.abilities)
			{
				...
				if (show == 0)
				{
					...
					for (size_t i = 0; i < COMBAT_COUNT; ++i)
					{
						...
						s << getCombatName(indexToCombatType(i)) << " field " << std::showpos << it.abilities->fieldAbsorbPercent[i]
							<< std::noshowpos << '%';
					}
				}
				else
...
bool Item::hasMarketAttributes() const
{
	...
	for (const auto& attr : attributes->getList())
	{
		...
		{
			const uint16_t charges = static_cast<uint16_t>(attr.value.integer);
		...
		{
			const uint32_t duration = static_cast<uint32_t>(attr.value.integer);
	...
	if (items[id].imbuingSlots > 0)
	{
		...
		{
			auto item = const_cast<Item*>(this);
