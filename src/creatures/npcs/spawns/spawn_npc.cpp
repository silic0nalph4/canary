...
void SpawnNpc::startSpawnNpcCheck()
{
	...
	{
		checkSpawnNpcEvent = g_scheduler.addEvent(createSchedulerTask(getInterval(), [this] { checkSpawnNpc(); }));
	}
}
...
bool SpawnNpc::spawnNpc(uint32_t spawnId, NpcType* npcType, const Position& pos, Direction dir,
                        bool startup /*= false*/)
{
...
void SpawnNpc::scheduleSpawnNpc(uint32_t spawnId, spawnBlockNpc_t sb, uint16_t interval)
{
	...
	else
	{
		...
		g_scheduler.addEvent(createSchedulerTask(1400, [this, spawnId, sb, capture0 = interval -
			                                         NONBLOCKABLE_SPAWN_NPC_INTERVAL]
		                                         {
			                                         scheduleSpawnNpc(spawnId, sb, capture0);
		                                         }));
	}
}
