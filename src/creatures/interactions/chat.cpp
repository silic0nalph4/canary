...
void PrivateChatChannel::invitePlayer(const Player& player, Player& invitePlayer)
{
	...
	ss << player.getName() << " invites you to " << (player.getSex() == PLAYERSEX_FEMALE ? "her" : "his") <<
...
bool ChatChannel::addUser(Player& player)
{
	...
	if (id == CHANNEL_GUILD)
	{
		...
		{
			g_scheduler.addEvent(createSchedulerTask(150, [ObjectPtr = &g_game, capture0 = player.getID()]
			{
...
bool Chat::load()
{
	...
	for (auto channelNode : doc.child("channels").children())
	{
		...
		const std::string channelName = channelNode.attribute("name").as_string();
	...
	case CHANNEL_PRIVATE:
		{
			...
			{
				const auto ret = privateChannels.emplace(
