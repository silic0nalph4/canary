			...
			if (!resultList.empty()) {
				...
				if (++it != resultList.end()) {
					...
					do {
						...
						const int32_t distance = std::max<int32_t>(Position::getDistanceX(myPos, pos),
...
bool Monster::canUseAttack(const Position& pos, const Creature* target) const {
	if (isHostile()) {
		...
		const uint32_t distance = std::max<uint32_t>(Position::getDistanceX(pos, targetPos),
...
bool Monster::canUseSpell(const Position& pos, const Position& targetPos,
	...
	if (sb.range != 0 && std::max<uint32_t>(Position::getDistanceX(pos, targetPos),
...
bool Monster::pushItem(Item* item) {
	...
	for (const auto& it : relList) {
		...
		if (tile && g_game.canThrowObjectTo(centerPos, tryPos)) {
			if (g_game.internalMoveItem(item->getParent(), tile, INDEX_WHEREEVER, item, item->getItemCount(), nullptr) ==
				RETURNVALUE_NOERROR) {
				return true;
			}
		}
...
bool Monster::getDistanceStep(const Position& targetPos, Direction& moveDirection, bool flee /* = false */) {
	...
	if (!flee && (distance > targetDistance || !g_game.isSightClear(creaturePos, targetPos, true))) {
		...
		return true;
		// we don't really care here, since it's what we wanted to reach (a dancestep will take of dancing in that position)
	}
	...
	if (offsetx == 0 && offsety == 0) {
		return getRandomStep(creaturePos, moveDirection);
...
bool Monster::isInSpawnRange(const Position& pos) const {
	...
	if (!SpawnsMonster::isInZone(masterPos, despawnRadius, pos)) {
		return false;
	}
...
void Monster::getPathSearchParams(const Creature* creature, FindPathParams& fpp) const {
	...
	if (isSummon()) {
		if (getMaster() == creature) {
			const int32_t distX = Position::getDistanceX(getPosition(), creature->getPosition());
			const int32_t distY = Position::getDistanceY(getPosition(), creature->getPosition());
