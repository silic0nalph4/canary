...
public:
	...
	void addBestiaryTrackerList(MonsterType* mtype) {
		if (client) {
			const auto it = std::find(BestiaryTracker.begin(), BestiaryTracker.end(), mtype);
	...
	void refreshBestiaryTracker(std::list<MonsterType*> trackerList) {
		if (client) {
			client->refreshBestiaryTracker(std::move(trackerList));
		}
	}
	...
	void addItemOnStash(uint16_t clientId, uint32_t amount) {
		const auto it = stashItems.find(clientId);
	...
	void sendAddTileItem(const Tile* itemTile, const Position& pos, const Item* item) {
		if (client) {
			const int32_t stackpos = itemTile->getStackposOfItem(this, item);
			if (stackpos != -1) {
	...
	void sendUpdateTileItem(const Tile* updateTile, const Position& pos, const Item* item) {
		if (client) {
			const int32_t stackpos = updateTile->getStackposOfItem(this, item);
			if (stackpos != -1) {
	...
	void sendLockerItems(std::map<uint16_t, uint16_t> itemMap, uint16_t count) {
		if (client) {
			client->sendLockerItems(std::move(itemMap), count);
		}
	}
	...
	bool isQuickLootListedItem(const Item* item) const {
		...
		const auto it = std::find(quickLootListClientIds.begin(),
	...
	void updateInputAnalyzer(CombatType_t type, int32_t amount, std::string target) {
		if (client) {
			client->sendUpdateInputAnalyzer(type, amount, std::move(target));
		}
	}
	...
	uint16_t parseRacebyCharm(charmRune_t charmId, bool set, uint16_t newRaceid) {
		...
		switch (charmId) {
		case CHARM_WOUND:
			if (set) { charmRuneWound = newRaceid; }
			else { raceid = charmRuneWound; }
			break;
		case CHARM_ENFLAME:
			if (set) { charmRuneEnflame = newRaceid; }
			else { raceid = charmRuneEnflame; }
			break;
		case CHARM_POISON:
			if (set) { charmRunePoison = newRaceid; }
			else { raceid = charmRunePoison; }
			break;
		case CHARM_FREEZE:
			if (set) { charmRuneFreeze = newRaceid; }
			else { raceid = charmRuneFreeze; }
			break;
		case CHARM_ZAP:
			if (set) { charmRuneZap = newRaceid; }
			else { raceid = charmRuneZap; }
			break;
		case CHARM_CURSE:
			if (set) { charmRuneCurse = newRaceid; }
			else { raceid = charmRuneCurse; }
			break;
		case CHARM_CRIPPLE:
			if (set) { charmRuneCripple = newRaceid; }
			else { raceid = charmRuneCripple; }
			break;
		case CHARM_PARRY:
			if (set) { charmRuneParry = newRaceid; }
			else { raceid = charmRuneParry; }
			break;
		case CHARM_DODGE:
			if (set) { charmRuneDodge = newRaceid; }
			else { raceid = charmRuneDodge; }
			break;
		case CHARM_ADRENALINE:
			if (set) { charmRuneAdrenaline = newRaceid; }
			else { raceid = charmRuneAdrenaline; }
			break;
		case CHARM_NUMB:
			if (set) { charmRuneNumb = newRaceid; }
			else { raceid = charmRuneNumb; }
			break;
		case CHARM_CLEANSE:
			if (set) { charmRuneCleanse = newRaceid; }
			else { raceid = charmRuneCleanse; }
			break;
		case CHARM_BLESS:
			if (set) { charmRuneBless = newRaceid; }
			else { raceid = charmRuneBless; }
			break;
		case CHARM_SCAVENGE:
			if (set) { charmRuneScavenge = newRaceid; }
			else { raceid = charmRuneScavenge; }
			break;
		case CHARM_GUT:
			if (set) { charmRuneGut = newRaceid; }
			else { raceid = charmRuneGut; }
			break;
		case CHARM_LOW:
			if (set) { charmRuneLowBlow = newRaceid; }
			else { raceid = charmRuneLowBlow; }
			break;
		case CHARM_DIVINE:
			if (set) { charmRuneDivine = newRaceid; }
			else { raceid = charmRuneDivine; }
			break;
		case CHARM_VAMP:
			if (set) { charmRuneVamp = newRaceid; }
			else { raceid = charmRuneVamp; }
			break;
		case CHARM_VOID:
			if (set) { charmRuneVoid = newRaceid; }
			else { raceid = charmRuneVoid; }
			break;
		default:
			raceid = 0;
			break;
