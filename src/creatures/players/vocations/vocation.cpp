...
uint64_t Vocation::getReqSkillTries(uint8_t skill, uint16_t level)
{
	...
	const uint64_t tries = static_cast<uint64_t>(skillBase[skill] * std::pow(
		static_cast<double>(skillMultipliers[skill]), level - 11));
...
uint64_t Vocation::getReqMana(uint32_t magLevel)
{
	...
	const uint64_t reqMana = std::floor<uint64_t>(
