		...
		if (strcasecmp(baseNode.name(), "base") == 0) {
			...
			for (auto childNode : baseNode.children()) {
				...
				if (strcasecmp(type.c_str(), "item") == 0) {
					...
					auto it2 = std::find_if(imb.items.begin(), imb.items.end(),
...
BaseImbue* Imbuements::getBaseByID(uint16_t id) {
	const auto it = std::find_if(bases.begin(), bases.end(), [id](const BaseImbue& group_imb) {
...
Category* Imbuements::getCategoryByID(uint16_t id) {
	const auto it = std::find_if(categories.begin(), categories.end(), [id](const Category& cat_imb) {
