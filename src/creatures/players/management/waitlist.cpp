...
bool WaitingList::clientLogin(const Player* player) {
	...
	const uint32_t maxPlayers = static_cast<uint32_t>(g_config.getNumber(MAX_PLAYERS));
	if (maxPlayers == 0 || (info->priorityWaitList.empty() && info->waitList.empty() && g_game.getPlayersOnline() <
		maxPlayers)) {
		return true;
	}
