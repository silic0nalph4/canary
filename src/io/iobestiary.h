...
public:
	...
	Charm(std::string initname, charmRune_t initcharmRune_t, std::string
         initdescription, charm_t inittype, uint16_t initpoints, int32_t initbinary) :
			name(initname),
			id(initcharmRune_t),
...
public:
	...
	std::map<uint16_t, uint32_t> getBestiaryKillCountByMonsterIDs(Player* player, const std::map<uint16_t, std::string>&
	                                                              mtype_list) const;
	std::map<uint8_t, int16_t> getMonsterElements(MonsterType* mtype) const;
	std::map<uint16_t, std::string> findRaceByName(const std::string& race, bool Onlystring = true,
