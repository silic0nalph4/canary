...
private:
	...
	void sendHighscores(const std::vector<HighscoreCharacter>& characters, uint8_t categoryId, uint32_t vocationId,
	...
	void refreshBestiaryTracker(const std::list<MonsterType*>& trackerList);
	...
	// Unjust Panel
	void sendUnjustifiedPoints(const uint8_t& dayProgress, const uint8_t& dayLeft, const uint8_t& weekProgress,
	                           const uint8_t& weekLeft, const uint8_t& monthProgress, const uint8_t& monthLeft,
	...
	void sendStorePurchaseSuccessful(const std::string& message, uint32_t coinBalance);
	...
	//analyzers
	void sendKillTrackerUpdate(Container* corpse, const std::string& name, Outfit_t creatureOutfit);
